    ...
    public partial class Window : Form
    {
      ...
      /*     if (!Minimized)
        ...
        private void Window_Load(object sender, EventArgs e) {
            ...
            this.Icon = new Icon(System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("MCWorlds.Lawl.ico"));
            ...
            s.Start();
            notifyIcon1.Text = ("MCWorlds Server: " + Server.name);
        ...
        void SettingsUpdate()
        {
            ...
            if (txtLog.InvokeRequired)
            {
                ...
                this.Text = Server.name + " MCWorlds Version: " + Server.Version;
            }
        }
        ...
        public void UpdateClientList(List<Player> players) {
            if (this.InvokeRequired) {
                ...
                {
                    if (p.falseName != "") { liClients.Items.Add(p.name + "(" + p.falseName + ")"); }
                    else { liClients.Items.Add(p.name); }
                });
            }
        }
        ...
        public void UpdateMapList(string blah) {            
            if (this.InvokeRequired) {
                ...
                {
                    liMaps.Items.Add(Server.levels[i].name + " (" + Server.levels[i].world + ") - " + Server.levels[i].physics);
                }
            }
        }
        ...
        private void tmrRestart_Tick(object sender, EventArgs e)
        {
            if (Server.autorestart)
            {
                if (DateTime.Now.TimeOfDay.CompareTo(Server.restarttime.TimeOfDay) > 0 && (DateTime.Now.TimeOfDay.CompareTo(Server.restarttime.AddSeconds(1).TimeOfDay)) < 0) {
                    Player.GlobalMessage("Il est " + DateTime.Now.TimeOfDay);
                    Player.GlobalMessage("Le serveur vas se redemarrer automatiquement.");
                    ...
                    MCWorlds_.Gui.Program.ExitProgram(true);
                }
            }
        }
