...
namespace MCWorlds
{
    public class Group
    {
        ...
        public Group(LevelPermission Perm, int maxB, string fullName, char newColor, string file, string newStatut) {
            Permission = Perm;
            maxBlocks = maxB;
            trueName = fullName;
        ...
        public static void InitAll()
        {
                                ...
                                else
                                {
                                    switch (property.ToLower())
                                    {
                                        ...
                                        case "statut":
                                            {
                                                if (value.Length > 16)
                                                { Server.s.Log("Le statut doit faire moins de 16 caracteres de long"); }
                                                else
                                                {
                                                    thisGroup.statut = value;
                                                    gots++;
                                                }
                                                break;
                                            }
                                    ...
                                    if (gots >= 4)
                                    {
                                        GroupList.Add(new Group(thisGroup.Permission, thisGroup.maxBlocks, thisGroup.trueName, thisGroup.color[0], thisGroup.fileName, thisGroup.statut));
                                    }
                                }
                            }
                            else
            ...
            if (GroupList.Find(grp => grp.Permission == LevelPermission.Operator) == null) GroupList.Add(new Group(LevelPermission.Operator, 2500, "Operator", 'c', "operators.txt", "MODO"));
            if (GroupList.Find(grp => grp.Permission == LevelPermission.Admin) == null) GroupList.Add(new Group(LevelPermission.Admin, 65536, "SuperOP", 'e', "uberOps.txt", "ADMIN"));
            ...
            else standard = Group.findPerm(LevelPermission.Builder);
        ...
        public static void saveGroups(List<Group> givenList)
        {
            ...
            foreach (Group grp in givenList)
            {
                if (grp.name != "nobody")
                {
                    ...
                    SW.WriteLine("Statut = " + grp.statut);
        ...
        public static string concatList(bool includeColor = true, bool skipExtra = false, bool permissions = false)
        {
            ...
            foreach (Group grp in Group.GroupList)
            {
                if (!skipExtra || (grp.Permission > LevelPermission.Banned && grp.Permission < LevelPermission.Nobody))
                    if (includeColor) {
