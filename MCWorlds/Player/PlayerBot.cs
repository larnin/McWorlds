...
namespace MCWorlds
{
    public sealed class PlayerBot
    {
        ...
        public DateTime lastSpeek = DateTime.Now;
        ...
        public System.Timers.Timer jumpTimer = new System.Timers.Timer(20);

        #region == constructors ==
        public PlayerBot(string n, Level l, string w, ushort x, ushort y, ushort z, byte rotx, byte roty)
        {
            name = n;
            color = "&1";
            id = FreeId();
            ...
            foreach (Player p in Player.players)
            {
                if (p.level == level)
                {
                    Player.SendMessage(p, "Le bot " + color + name + Server.DefaultColor + " a ete ajoute.");
                }
            }
            ...
            botTimer.Elapsed += delegate
            {
                ...
                if (sayMessage != "")
                {
                    if (lastSpeek.AddSeconds(5) <= DateTime.Now)
                    {
                        bool say = false;
                        foreach (Player pl in Player.players)
                        {
                            if (pl.level != level) { continue; }
                            if (pos[0] == pl.pos[0] && pos[1] == pl.pos[1] && pos[2] == pl.pos[2])
                            { Player.SendMessage(pl, color + name + ": &f" + sayMessage); say = true; continue; }

                            double dist = Math.Sqrt((pos[0] - pl.pos[0]) * (pos[0] - pl.pos[0]) + (pos[1] - pl.pos[1]) * (pos[1] - pl.pos[1]) + (pos[2] - pl.pos[2]) * (pos[2] - pl.pos[2]));
                            if (dist > 32 * 2) { continue; } // a plus de 2 blocs
                            int xp = pl.pos[0] - pos[0];
                            int yp = pl.pos[1] - pos[1];
                            int zp = pl.pos[2] - pos[2];
                            double dh = Math.Sqrt(xp * xp + zp * zp);

                            double angleH = Math.Atan2(zp, xp) / Math.PI * 128 + 64;
                            if (angleH > 255) { angleH -= 256; }
                            double angleV = Math.Atan2(yp, dh) / Math.PI * 128;

                            double dAH = Math.Abs(angleH - rot[0]);
                            double dAV = Math.Abs(rot[1] - angleV);

                            if ((dAH < 32 || dAH > 224) && (dAV < 32 || dAV > 224)) { Player.SendMessage(pl, color + name + ": &f" + sayMessage); say = true; }
                        }
                        if (say) { lastSpeek = DateTime.Now; }
                    }
                }
