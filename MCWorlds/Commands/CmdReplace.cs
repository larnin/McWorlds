...
namespace MCWorlds
{
    public class CmdReplace : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            if (!Block.canPlace(p, type) && !Block.BuildIn(type)) { Player.SendMessage(p, "Impossible de remplacer ca."); return; }

            if (!Block.canPlace(p, type2)) { Player.SendMessage(p, "Impossible de placer ca."); return; }
        ...
        {
            Player.SendMessage(p, "/replace [type] [type2] - Remplace [type] en [type2] dans la zone definie");
        }
