    ...
    public class CmdMissile : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            if (p.aiming)
            {
                ...
                {
                    p.aiming = false;
                    Player.SendMessage(p, "Missiles desactive");
                    return;
                }
            }
            ...
            p.SendMessage("Missile active, Tire et dirige !");
                                    ...
                                    if ((ushort)(pl.pos[1] / 32) == pos.y || (ushort)(pl.pos[1] / 32 + 1) == pos.y || (ushort)(pl.pos[1] / 32 - 1) == pos.y)
                                    {
                                        if ((ushort)(pl.pos[2] / 32) == pos.z || (ushort)(pl.pos[2] / 32 + 1) == pos.z || (ushort)(pl.pos[2] / 32 - 1) == pos.z)
                                        {
                                            if (p.level.physics >= 3 && bp.ending >= 2)
                                                ...
                                                pl.HandleDeath(Block.stone, " s'est fait touche par un missile de " + p.color + p.Name());
                                            comeOut = true;
                                        }
                                    }
                                }
        ...
        {
            Player.SendMessage(p, "/missile [parametre] - Vous permet de tirer des missiles sur des joueurs");
            Player.SendMessage(p, "[parametres] valides: &cexplode, destroy");
        }
