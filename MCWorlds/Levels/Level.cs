...
namespace MCWorlds
{
    ...
    public class Level
    {
        ...
        // changements
        public string world;
        public List<string> Perbuildliste = new List<string>();
        public bool perbuild = false;
        public bool pervisit = true;
        public bool pergun = false;
                //public script scriptEnCours = null;
        ...
        public bool Unload()
        {
            ...
            {
                if (pl.level == this) Command.all.Find("goto").Use(pl, Server.mainLevel.name + " " + Server.mainWorld);
            });
            ...
            BaseGame game = Server.allGames.Find(g => g.lvl == this);
            if (game != null)
            {
                if (game.gameOn)
                { game.stopGame(null); }
            ...
            {
                if (pl.level.world == world) { Player.SendMessage(pl, "Map \"" + name + "\" n'est plus charge"); }
            }
        ...
        public void saveChanges()
        {
            ...
            string queryString;
            queryString = "INSERT INTO `Block" + name + "." + world + "` (Username, TimePerformed, X, Y, Z, type, deleted) VALUES ";
        ...
        public static Level Find(string levelName, string world)
        {
            Level tempLevel = null; bool returnNull = false;

            for (int i = 0 ; i < Server.levels.Count ; i++)
            {
                if (Server.levels[i].name.ToLower() == levelName && Server.levels[i].world.ToLower() == world) return Server.levels[i];
                if (Server.levels[i].name.ToLower().IndexOf(levelName.ToLower()) != -1 && Server.levels[i].world.ToLower() == world)
                {
                    if (tempLevel == null) tempLevel = Server.levels[i];
        ...
        public static Level FindExact(string levelName, string worldName)
        {
            return Server.levels.Find(lvl => ( levelName.ToLower() == lvl.name.ToLower() &&  worldName.ToLower() == lvl.world.ToLower()));
        }
    ...
    retry:  try
            {
                ...
                if (!p.perbuild && p.group.Permission < LevelPermission.Admin)
                {
                    p.SendBlockchange(x, y, z, b);
                    Player.SendMessage(p, "Vous ne pouvez pas construire ici");
                    return;
                }
                ...
                Player.UndoPos Pos;
                Pos.x = x; Pos.y = y; Pos.z = z; Pos.mapName = name; Pos.worldName = world;
            ...
            {
                Player.SendMessage(p, "Le buffer dedie a undo est plein! Supression!");
                p.UndoBuffer.Clear();
                goto retry;
            }
            catch (Exception e)
            {
                Server.ErrorLog(e);
                Player.GlobalMessageOps(p.name + " a cree une erreur non-fatal a " + name);
                Player.GlobalMessageOps("Localisation de l'erreur: " + errorLocation);
        ...
        {
            string path = "levels/" + world + "/" + name + ".lvl";

            try
            {
                if (!Directory.Exists("levels")) Directory.CreateDirectory("levels");
                if (!Directory.Exists("levels/" + world)) Directory.CreateDirectory("levels/" + world);
                if (!Directory.Exists("levels/" + world + "/level properties")) Directory.CreateDirectory("levels/" + world + "/level properties");
                ...
                if (changed == true || !File.Exists(path) || Override)
                {
                    ...
                    BitConverter.GetBytes(1874).CopyTo(header, 0); //ancien 1874, pour eviter que les maps soient réutilise
                    gs.Write(header, 0, 2);
                    ...
                    StreamWriter SW = new StreamWriter(File.Create("levels/" + world + "/level properties/" + name + ".properties"));
                    ...
                    SW.WriteLine("Unload = " + unload.ToString());
                    
                    SW.WriteLine("PermBuild = " + perbuild.ToString());
                    SW.WriteLine("PermVisite = " + pervisit.ToString());
                    SW.WriteLine("PerGun = " + pergun.ToString());
                    string ns = "";
                    foreach (string n in Perbuildliste)
                    { ns += n + ","; }
                    if (ns != "")
                    { ns = ns.Remove(ns.Length - 1); }
                    SW.WriteLine("PerbuildListe = " + ns);

                    SW.Flush();
                    SW.Close();

                    Server.s.Log("SAVED: Level \"" + name + "(" + world + ")" + "\". (" + players.Count + "/" + Player.players.Count  + "/" + Server.players + ")");
                    changed = false;
            ...
            catch (Exception e)
            {
                ...
                Player.GlobalMessage("ERREUR A LA SAUVEGARDE DE :" + name);
        ...
        public int Backup(bool Forced = false, string backupName = "")
        {
            if (!backedup || Forced)
            {
                ...
                if (Directory.Exists(backupPath + "/" + world + "/" + name))
                {
                    backupNumber = Directory.GetDirectories(backupPath + "/" + world + "/" + name).Length + 1;
                }
                else
                {
                    Directory.CreateDirectory(backupPath + "/" + world + "/" + name);
                }
                string path = backupPath + "/" + world + "/" + name + "/" + backupNumber;
                if (backupName != "")
                {
                    path = backupPath + "/" + world + "/" + name + "/" + backupName;
                }
                Directory.CreateDirectory(path);

                string BackPath = path + "/" + name + ".lvl";
                string current = "levels/" + world + "/" + name + ".lvl";
                try
                {
        ...
        public static Level Load(string givenName, string world) { return Load(givenName,world, 0); }
        public static Level Load(string givenName, string world, byte phys)
        {
            MySQL.executeQuery("CREATE TABLE if not exists `Block" + givenName + "." + world + "` (Username CHAR(33), TimePerformed DATETIME, X SMALLINT UNSIGNED, Y SMALLINT UNSIGNED, Z SMALLINT UNSIGNED, Type TINYINT UNSIGNED, Deleted BOOL)");
            MySQL.executeQuery("CREATE TABLE if not exists `Portals" + givenName + "." + world + "` (EntryX SMALLINT UNSIGNED, EntryY SMALLINT UNSIGNED, EntryZ SMALLINT UNSIGNED, ExitMap CHAR(33), ExitX SMALLINT UNSIGNED, ExitY SMALLINT UNSIGNED, ExitZ SMALLINT UNSIGNED)");
            MySQL.executeQuery("CREATE TABLE if not exists `Messages" + givenName + "." + world + "` (X SMALLINT UNSIGNED, Y SMALLINT UNSIGNED, Z SMALLINT UNSIGNED, Message CHAR(255));");
            MySQL.executeQuery("CREATE TABLE if not exists `Like" + givenName + "." + world + "` (Name CHAR(33))");

            string path = "levels/" + world + "/" + givenName + ".lvl";
            if (File.Exists(path))
            {
                ...
                try
                {
                    ...
                    try
                    {
                        DataTable foundDB = MySQL.fillData("SELECT * FROM `Portals" + givenName + "." + world + "`");
                        ...
                        for (int i = 0; i < foundDB.Rows.Count; ++i)
                        {
                            ...
                            {
                                MySQL.executeQuery("DELETE FROM `Portals" + givenName + "." + world + "` WHERE EntryX=" + foundDB.Rows[i]["EntryX"] + " AND EntryY=" + foundDB.Rows[i]["EntryY"] + " AND EntryZ=" + foundDB.Rows[i]["EntryZ"]);
                            }
                        }

                        foundDB = MySQL.fillData("SELECT * FROM `Messages" + givenName + "." + world + "`");

                        for (int i = 0; i < foundDB.Rows.Count; ++i)
                        {
                            if (!Block.mb(level.GetTile((ushort)foundDB.Rows[i]["X"], (ushort)foundDB.Rows[i]["Y"], (ushort)foundDB.Rows[i]["Z"])))
                            {
                                MySQL.executeQuery("DELETE FROM `Messages" + givenName + "." + world + "` WHERE X=" + foundDB.Rows[i]["X"] + " AND Y=" + foundDB.Rows[i]["Y"] + " AND Z=" + foundDB.Rows[i]["Z"]);
                            }
                        }
                        foundDB.Dispose();
                    }
                    ...
                    try
                    {
                        ...
                        foundLocation = "levels/" + world + "/level properties/" + level.name + ".properties";
                        if (!File.Exists(foundLocation))
                        {
                            foundLocation = "levels/" + world + "/level properties/" + level.name;
                        }
                            ...
                            try
                            {
                                if (line[0] != '#')
                                {
                                    ...
                                    switch (line.Substring(0, line.IndexOf(" = ")).ToLower())
                                    {
                                        ...
                                        case "permbuild": level.perbuild = bool.Parse(value); break;
                                        case "permvisit": level.pervisit = bool.Parse(value); break;
                                        case "pergun": level.pergun = bool.Parse(value); break;
                                        case "perbuildliste":
                                            foreach( string name in value.Split(','))
                                            { level.Perbuildliste.Add(name); }
                                            break;   
                                    }
                                }
                            }
                    ...
                    Server.s.Log("Level \"" + level.name + "\", world \"" + level.world + "\" loaded.");
                    return level;
                }
        ...
        public void Physics()
        {
            ...
            while (true)
            {
                ...
                retry: if (wait > 0) Thread.Sleep(wait);
                    if (physics == 0 || ListCheck.Count == 0 || physPause) goto retry;
                    ...
                    if (wait < (int)(-overload * 0.75f))
                    {
                        ...
                        if (wait < -overload)
                        {
                            ...
                            Player.GlobalMessage("Les physiques s'arettent sur &b" + Cause.name + " du monde " + Cause.world);
                            Server.s.Log("Physics shutdown on " + name + ", world " + world );
                        ...
                        else
                        {
                            ...
                            {
                                if (p.level == this) Player.SendMessage(p, "Alertes physiques!");
                            }
                        ...
                        newPhysic: if (foundInfo != "")
                            ...
                            else
                            {
                                switch (blocks[C.b])
                                {
                                            ...
                                            for (int cy = (-1 * my); cy != ((1 * my) + my); cy = cy + (1 * my))
                                                for (int cz = (-1 * mz); cz != ((1 * mz) + mz); cz = cz + (1 * mz))
                                                {
                                                    if (GetTile((ushort)(x + cx), (ushort)(y + cy - 1), (ushort)(z + cz)) == Block.red && (GetTile((ushort)(x + cx), (ushort)(y + cy), (ushort)(z + cz)) == Block.air || GetTile((ushort)(x + cx), (ushort)(y + cy), (ushort)(z + cz)) == Block.water) && !InnerChange)
                                                    {
                                                        ...
                                                        AddUpdate(IntOffset(C.b, 0, -1, 0), Block.obsitrain, true, "wait 5 revert " + Block.red.ToString());
                                                    ...
                                                    else if (GetTile((ushort)(x + cx), (ushort)(y + cy - 1), (ushort)(z + cz)) == Block.op_air && (GetTile((ushort)(x + cx), (ushort)(y + cy), (ushort)(z + cz)) == Block.air || GetTile((ushort)(x + cx), (ushort)(y + cy), (ushort)(z + cz)) == Block.water) && !InnerChange)
                                                    {
                                                        AddUpdate(PosToInt((ushort)(x + cx), (ushort)(y + cy), (ushort)(z + cz)), Block.train);
                                                        AddUpdate(PosToInt(x, y, z), Block.air);
                                                        AddUpdate(IntOffset(C.b, 0, -1, 0), Block.op_glass, true, "wait 5 revert " + Block.op_air.ToString());
                                                        InnerChange = true;
                                                        break;
                                    ...
                                    case Block.ascenseur:
                                        for (int ax = -1; ax <= 1; ax++)
                                        {
                                            for (int az = -1; az <= 1; az++)
                                            {
                                                if (ax * az != 0 || ax + az == 0) { continue; }

                                                for (int ay = -1; ay <= 1; ay+=2)
                                                {
                                                    if (GetTile((ushort)(x + ax), (ushort)(y + ay), (ushort)(z + az)) == Block.obsidian && GetTile(x, (ushort)(y + ay), z) == Block.air && !InnerChange)
                                                    { goto action; }
                                                    else { continue; }

                                                action:
                                                    {
                                                        AddUpdate(IntOffset(C.b, 0, ay, 0), Block.ascenseur);
                                                        AddUpdate(C.b, Block.air);
                                                        for (int bx = -1; bx <= 1; bx++)
                                                        {
                                                            for (int bz = -1; bz <= 1; bz++)
                                                            {
                                                                if (bx * bz != 0 || bx + bz == 0) { continue; }
                                                                if (GetTile((ushort)(x + bx), y, (ushort)(z + bz)) == Block.obsidian)
                                                                { AddUpdate(IntOffset(C.b, bx, 0, bz), Block.blue, true, "wait 5 revert " + Block.obsidian.ToString()); }
                        
                                                                if (GetTile(x, (ushort)(y + 4 + ay), z) == Block.air && (GetTile((ushort)(x + bx), (ushort)(y + 4 + ay), (ushort)(z + bz)) == Block.obsidian || GetTile((ushort)(x + bx), (ushort)(y + 4 + ay), (ushort)(z + bz)) == Block.blue ) )
                                                                { AddUpdate(IntOffset(C.b, 0, 4 + ay, 0), Block.staircasestep, false, "dissipate 100"); }
                                                            }
                                                        }
        ...
        public void PhysDoor(ushort x, ushort y, ushort z, bool instaUpdate)
        {
            ...
            if (blocks[foundInt] == Block.redstone_off) { AddUpdate(foundInt, Block.redstone_on, true, "wait 15 revert " + Block.redstone_off.ToString()); }

            if (blocks[foundInt] == Block.doorin)
            {
                int numBloc = 0;
                byte aird = (byte)0;
                for ( int i = -2 ; i <= 2 ; i++ )
                {
                    for ( int j = -2 ; j <= 2 ; j++)
                    {
                        if (Math.Abs(i) + Math.Abs(j) == 2)
                        {
                            if (i == 0 || j == 0)
                            {
                                if (blocks[PosToInt((ushort)(x + i / 2), y, z)] == Block.doorout || blocks[PosToInt(x, y, (ushort)(z + j / 2))] == Block.doorout)
                                {
                                    numBloc = PosToInt((ushort)(x + i), y, (ushort)(z + j));
                                    aird = Block.DoorAirs(blocks[numBloc]);

                                    if (aird != 0)
                                    {
                                        if (!instaUpdate) AddUpdate(numBloc, aird);
                                        else Blockchange((ushort)(x + i), y, (ushort)(z + j), aird);
                                    }
                                }
                            }
                            else
                            {
                                if (blocks[PosToInt((ushort)(x + i), y, z)] == Block.doorout || blocks[PosToInt(x, y, (ushort)(z + j))] == Block.doorout)
                                {
                                    numBloc = PosToInt((ushort)(x + i), y, (ushort)(z + j));
                                    aird = Block.DoorAirs(blocks[numBloc]);
        ...
        public void AnyRedstone(Check C, ushort x, ushort y, ushort z)
        {
            if (C.time < 255)
            {
                byte blocActuel = blocks[C.b];
                
                try { PhysRedstone(blocActuel, (ushort)(x + 1), y, z); }
                catch { }
                try { PhysRedstone(blocActuel, (ushort)(x - 1), y, z); }
                catch { }
                try { PhysRedstone(blocActuel, x, y, (ushort)(z + 1)); }
                catch { }
                try { PhysRedstone(blocActuel, x, y, (ushort)(z - 1)); }
                catch { }
                try { PhysRedstone(blocActuel, x, (ushort)(y - 1), z); }
                catch { }
                try { PhysRedstone(blocActuel, x, (ushort)(y + 1), z); }
                catch { }
                if (C.extraInfo == "" && C.time < 255) { C.time = 255; }
                else { C.time++; }
            }
            else
            {
        ...
        public void PhysRedstone(byte b, ushort x, ushort y, ushort z)
        {
            byte bA = GetTile(x, y, z);
            int foundInt = PosToInt(x, y, z);


            if (b == Block.redstone_off)
            {
                if (bA == Block.redstone_on) { AddUpdate(foundInt, Block.redstone_off, true); }

                if (bA == Block.red_torche_on)
                {
                    if (GetTile(IntOffset(foundInt, 1, 0, 0)) == Block.redstone_off) { PhysRedstone(Block.redstone_on, (ushort)(x + 1), y, z); }
                    if (GetTile(IntOffset(foundInt, -1, 0, 0)) == Block.redstone_off) { PhysRedstone(Block.redstone_on, (ushort)(x - 1), y, z); }
                    if (GetTile(IntOffset(foundInt, 0, 0, 1)) == Block.redstone_off) { PhysRedstone(Block.redstone_on, x, y, (ushort)(z + 1)); }
                    if (GetTile(IntOffset(foundInt, 0, 0, -1)) == Block.redstone_off) { PhysRedstone(Block.redstone_on, x, y, (ushort)(z - 1)); }
                    if (GetTile(IntOffset(foundInt, 0, 1, 0)) == Block.redstone_off) { PhysRedstone(Block.redstone_on, x, (ushort)(y - 1), z); }
                    if (GetTile(IntOffset(foundInt, 0, -1, 0)) == Block.redstone_off) { PhysRedstone(Block.redstone_on, x, (ushort)(y + 1), z); }
                }
                
                if (Block.AnyBuild(bA) && !Block.Walkthrough(bA) && !Block.LightPass(bA) && !Block.isRedstone(bA))
                {
                    if (GetTile(IntOffset(foundInt, 0, 0, -1)) == Block.red_torche_off) { AddCheck(IntOffset(foundInt, 0, 0, -1), "", true); }
                    if (GetTile(IntOffset(foundInt, 0, 0, 1)) == Block.red_torche_off) { AddCheck(IntOffset(foundInt, 0, 0, 1), "", true); }
                    if (GetTile(IntOffset(foundInt, 0, -1, 0)) == Block.red_torche_off) { AddCheck(IntOffset(foundInt, 0, -1, 0), "", true); }
                    if (GetTile(IntOffset(foundInt, 0, 1, 0)) == Block.red_torche_off) { AddCheck(IntOffset(foundInt, 0, 1, 0), "", true); }
                    if (GetTile(IntOffset(foundInt, -1, 0, 0)) == Block.red_torche_off) { AddCheck(IntOffset(foundInt, -1, 0, 0), "", true); }
                    if (GetTile(IntOffset(foundInt, 1, 0, 0)) == Block.red_torche_off) { AddCheck(IntOffset(foundInt, 1, 0, 0), "", true); }
                }
            }
            if (b == Block.redstone_on)
            {
                if (bA == Block.redstone_off) { AddUpdate(foundInt, Block.redstone_on, true); }

                if (Block.AnyBuild(bA) && !Block.Walkthrough(bA) && !Block.LightPass(bA) && !Block.isRedstone(bA))
                {
                    if (GetTile(IntOffset(foundInt, 0, 0, -1)) == Block.red_torche_on) { AddCheck(IntOffset(foundInt, 0, 0, -1), "", true); }
                    if (GetTile(IntOffset(foundInt, 0, 0, 1)) == Block.red_torche_on) { AddCheck(IntOffset(foundInt, 0, 0, 1), "", true); }
                    if (GetTile(IntOffset(foundInt, 0, -1, 0)) == Block.red_torche_on) { AddCheck(IntOffset(foundInt, 0, -1, 0), "", true); }
                    if (GetTile(IntOffset(foundInt, 0, 1, 0)) == Block.red_torche_on) { AddCheck(IntOffset(foundInt, 0, 1, 0), "", true); }
                    if (GetTile(IntOffset(foundInt, -1, 0, 0)) == Block.red_torche_on) { AddCheck(IntOffset(foundInt, -1, 0, 0), "", true); }
                    if (GetTile(IntOffset(foundInt, 1, 0, 0)) == Block.red_torche_on) { AddCheck(IntOffset(foundInt, 1, 0, 0), "", true); }
                }

                if (Block.tDoor(blocks[foundInt]))
                { AddUpdate(foundInt, Block.air, false, "wait 16 door 1 revert " + blocks[foundInt].ToString()); } //tdoor

                if (Block.odoor(blocks[foundInt]) != Block.Zero) AddUpdate(foundInt, Block.odoor(blocks[foundInt]), true); //odoor

                byte FoundAirDoor = Block.DoorAirs(blocks[foundInt]);
                if (FoundAirDoor != 0) { AddUpdate(foundInt, FoundAirDoor); } //door

                if (bA == Block.piston_body) { AddCheck(foundInt,"",true); }
            }
        }

        public void PhysRedTorche(Check C, ushort x, ushort y, ushort z)
        {
            byte b = blocks[C.b];
            if (b != Block.red_torche_off && b != Block.red_torche_on) { C.time = 255; return; }

            if (C.time == 0) { C.time++; return; }
            ...
            for (int i = -1; i <= 1; i++)
            {
                for (int j = -1; j <= 1; j++)
                {
                    for (int k = -1; k <= 1; k++)
                    {
                        ...
                        byte bSolide = GetTile((ushort)(x + i), (ushort)(y + j), (ushort)(z + k));
                        if (!Block.AnyBuild(bSolide) || Block.Walkthrough(bSolide) || Block.LightPass(bSolide) || Block.isRedstone(bSolide)) { continue; }
                        ...
                        for (int l = -1; l <= 1; l++)
                        {
                            for (int m = -1; m <= 1; m++)
                            {
                                for (int n = -1; n <= 1; n++)
                                {
                                    ...
                                    if (GetTile((ushort)(x + i + l), (ushort)(y + j + m), (ushort)(z + k + n)) == Block.redstone_on) { redOn = true; }
                                }
                            }
                        }
                    }
            ...
            if (redOn)
            {
                Blockchange(x, y, z, Block.red_torche_off, true);
                if (GetTile((ushort)(x + 1), y, z) == Block.redstone_on) { AddUpdate(PosToInt((ushort)(x + 1), y, z), Block.redstone_off, true); }
                if (GetTile((ushort)(x - 1), y, z) == Block.redstone_on) { AddUpdate(PosToInt((ushort)(x - 1), y, z), Block.redstone_off, true); }
                if (GetTile(x, (ushort)(y + 1), z) == Block.redstone_on) { AddUpdate(PosToInt(x, (ushort)(y + 1), z), Block.redstone_off, true); }
                if (GetTile(x, (ushort)(y - 1), z) == Block.redstone_on) { AddUpdate(PosToInt(x, (ushort)(y - 1), z), Block.redstone_off, true); }
                if (GetTile(x, y, (ushort)(z + 1)) == Block.redstone_on) { AddUpdate(PosToInt(x, y, (ushort)(z + 1)), Block.redstone_off, true); }
                if (GetTile(x, y, (ushort)(z - 1)) == Block.redstone_on) { AddUpdate(PosToInt(x, y, (ushort)(z - 1)), Block.redstone_off, true); }
            }
            else
            {
                Blockchange(x, y, z, Block.red_torche_on, true);
                if (GetTile((ushort)(x + 1), y, z) == Block.redstone_off) { AddUpdate(PosToInt((ushort)(x + 1), y, z), Block.redstone_on, true); }
                if (GetTile((ushort)(x - 1), y, z) == Block.redstone_off) { AddUpdate(PosToInt((ushort)(x - 1), y, z), Block.redstone_on, true); }
                if (GetTile(x, (ushort)(y + 1), z) == Block.redstone_off) { AddUpdate(PosToInt(x, (ushort)(y + 1), z), Block.redstone_on, true); }
                if (GetTile(x, (ushort)(y - 1), z) == Block.redstone_off) { AddUpdate(PosToInt(x, (ushort)(y - 1), z), Block.redstone_on, true); }
                if (GetTile(x, y, (ushort)(z + 1)) == Block.redstone_off) { AddUpdate(PosToInt(x, y, (ushort)(z + 1)), Block.redstone_on, true); }
                if (GetTile(x, y, (ushort)(z - 1)) == Block.redstone_off) { AddUpdate(PosToInt(x, y, (ushort)(z - 1)), Block.redstone_on, true); }
        ...
        public void PhysPiston(Check C, ushort x, ushort y, ushort z)
        {
...
public class Check
{
    ...
    public Check(int bl, string extra = "")
    {
        b = bl;
        time = 0;
        extraInfo = extra;
...
public class Update
{
    ...
    public byte type;
    public string extraInfo = "";
    public Update(int bl, byte typ, string extra = "")
    {
        b = bl;
        type = typ;
        extraInfo = extra;
